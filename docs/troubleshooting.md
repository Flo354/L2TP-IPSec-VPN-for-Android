...
| Symptom | Cause | Fix |
| --- | --- | --- |
| **No reply at all to main mode message 1**, and `Local address for this tunnel` is on the router's *own* LAN (`192.168.1.x` when the router is `192.168.1.1`) | **NAT hairpinning.** The phone is on the router's Wi-Fi and is trying to reach the router's *public* address from inside. Most consumer routers, Liveboxes included, do not loop that traffic back to their own VPN service, so the packet simply vanishes | Not a client fault, and nothing to fix in the app. Connect over mobile data or from any network other than the router's own. This is also why testing a home VPN from the sofa is misleading |
